...
{
    if (std::find(parsed.begin(), parsed.end(), filepath) != parsed.end())
        return;
    parsed.push_back(filepath);

    std::ifstream stream(filepath);
    Parser parser(stream, filepath, parsed, callback);
    for (ExprPtr expression; expression = parser.GetNext();)
    {
        callback(expression);
    }
}
...
tpp::Token &tpp::Parser::Next()
{
    ...
    while (chr >= 0)
    {
        ...
        case ParserMode_Number:
            ...
            if (!isdigit(chr))
                return m_Token = {loc, TokenType_Number, value};
            value += (char)chr;
            break;

        case ParserMode_Id:
            if (!isalnum(chr) || chr == '_')
                return m_Token = {loc, TokenType_Id, value};
            value += (char)chr;
            break;

        case ParserMode_BinaryOperator:
            if (!isOp(chr))
                return m_Token = {loc, TokenType_BinaryOperator, value};
            value += (char)chr;
            break;
...
bool tpp::Parser::AtEOF()
{
    return m_Token.Type == TokenType_;
}

bool tpp::Parser::At(const TokenType type)
{
    return m_Token.Type == type;
}

bool tpp::Parser::At(const std::string &value)
{
...
void tpp::Parser::ParseInclude()
{
    ...
    if (!path.is_absolute())
        path = m_Filepath.parent_path() / filename;
...
void tpp::Parser::ParseNamespace()
{
    ...
    if (m_Namespace.empty() || m_Namespace.back() != name)
...
tpp::Name tpp::Parser::ParseName(bool inherit)
{
    ...
    while (NextIfAt(":"))
    {
        name += ':' + Expect(TokenType_Id).Value;
    }
...
tpp::ExprPtr tpp::Parser::ParseDef()
{
    ...
    if (NextIfAt("("))
    {
        ...
        return std::make_shared<DefFunctionExpression>(
            location,
            native_name,
            name,
            arg_names,
...
tpp::ExprPtr tpp::Parser::ParseBinary()
{
...
tpp::ExprPtr tpp::Parser::ParseMember()
{
...
tpp::ExprPtr tpp::Parser::ParsePrimary()
{
    ...
    if (NextIfAt("?"))
        return std::make_shared<VarArgsExpression>(location);
